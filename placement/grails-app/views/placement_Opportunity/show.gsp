	...
	<body>
		...
		<div id="show-placement_Opportunity" class="content scaffold-show" role="main">
			...
			<ol class="property-list placement_Opportunity">
				...
				<g:if test="${placement_OpportunityInstance?.application}">
				<li class="fieldcontain">
					<span id="application-label" class="property-label"><g:message code="placement_Opportunity.application.label" default="Application" /></span>
					
						<g:each in="${placement_OpportunityInstance.application}" var="a">
						<span class="property-value" aria-labelledby="application-label"><g:link controller="application" action="show" id="${a.id}">${a?.encodeAsHTML()}</g:link></span>
						</g:each>
